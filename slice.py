...
def slice(image, interactive=False, threshold_pxl_density=5.5, line_padding=7):
    ...
    computed_threshold, working_image = get_inverted_thresholded_image(working_image)
    ...
    while True:
        ...
        uppers = [y - 1 for y in range(H - 1) if hist[y] <= threshold_pxl_density < hist[y + 1]]
        lowers = [y + 1 for y in range(H - 1) if hist[y] > threshold_pxl_density >= hist[y + 1]]
        ...
        for y in uppers:
            ...
            cv2.line(display_image, (0, pt_y), (W, pt_y), BGR_UPPER[color_count % 2], 1, cv2.LINE_8)
            color_count += 1

        color_count = 0
        for y in lowers:
            pt_y = min(y + line_padding, H)
            cv2.line(display_image, (0, pt_y), (W, pt_y), BGR_LOWER[color_count % 2], 1, cv2.LINE_8)
            color_count += 1
